 ...
 * @property {string} birthdate - Birthdate in ISO 8601 format.
...
function getRandomBirthdate(minAge, maxAge) {
  ...
  // Define the minimum and maximum time in milliseconds for the birthdate based on the age range
...
export function main(dtoIn) {
  ...
  if (dtoIn.count <= 0 || dtoIn.age.min > dtoIn.age.max) {
    console.error("Neplatný vstup: Počet musí být > 0 a min věk <= max věk.");
    return [];
  }
